...
def minimumMoves(grid, startX, startY, goalX, goalY):
    ...
    def get_neighbors(indexX, indexY, direction, moves, grid):
        # neighbors array will take in the coordinates as (x, y)
        neighbors = []
        # if X value to the right of indexX is not the end of the passed in string 
        # or is not x 
        ...
        if (indexX + 1) < len(grid[0]): 
            if grid[indexX+1][indexY] != 'X':
                if direction == 'R':
                    neighbors.append((indexX+1, indexY, 'R', moves))
                else:
                    neighbors.append((indexX+1, indexY, 'R', moves+1))

        if (indexX - 1) >= 0:
            if grid[indexX-1][indexY] != 'X':
                if direction == 'L':
                    neighbors.append((indexX-1, indexY, 'L', moves))
                else:
                    neighbors.append((indexX-1, indexY, 'L', moves+1))

        if (indexY-1) >= 0:
            if grid[indexX][indexY-1] != 'X':
                if direction == 'U':
                    neighbors.append((indexX, indexY-1, 'U', moves))
                else:
                    neighbors.append((indexX, indexY-1, 'U', moves+1))

        if (indexY + 1) < len(grid):
            if grid[indexX][indexY+1] != 'X':
                if direction == 'D':
                    neighbors.append((indexX, indexY+1, 'D', moves))
                else:
                    neighbors.append((indexX, indexY+1, 'D', moves))
    ...
    queue = []
    queue.append((startX, startY, 'S', 0))
    result_path = []
    ...
    while len(queue) > 0:
        ...
        if (path[0], path[1]) not in visited:
            ...
            if path[0] == goalX and path[1] == goalY:
                return path[3] + 1
                
            for neighbor in get_neighbors(path[0], path[1], path[2], path[3], grid):
                # new_path = list(path)
                # new_path.append(neighbor) 
